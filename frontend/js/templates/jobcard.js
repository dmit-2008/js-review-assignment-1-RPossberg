...
export function jobCard(job) {
  const template = `
    <li class="job-card card my-1" style="width: 18rem;">
      <div class="card-header">${job.company || "Unknown Company"}</div>
      <div class="card-body">
        <h5 class="card-title">${job.title || "Unknown Title"}</h5>
        <h6 class="card-subtitle mb-2 text-body-secondary">${job.location || "Unknown Location"}</h6>
        <h6 class="card-subtitle mb-2 text-body-secondary">Posted ${job.date_posted ? new Date(job.date_posted).toLocaleDateString() : "Unknown Date"}</h6>
